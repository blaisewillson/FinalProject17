...
class Monster(object): #monster class
	...
	def fight(self, turn):  #fight function for monster
		global wait
		...
		if floor != 'village' and self.monsterhp > 0:
			if self.monsterX == Player.playerX + 1 and self.monsterY == Player.playerY or self.monsterX == Player.playerX and self.monsterY == Player.playerY -1 or self.monsterY == Player.playerY + 1 and self.monsterX == Player.playerX or self.monsterY == Player.playerY and self.monsterX == Player.playerX - 1:
				...
				while self.monsterhp > 0:
					...
					drawmap()
					if turn == 'monster':
						damage = random.randint(self.monsterlvl + self.basedmg1 + int(self.monsterlvl/2), self.monsterlvl + self.basedmg2 + int(self.monsterlvl/2))
						...
						if Player.health <= 0:   #if player health is zero they die
							screen.clear()
...
def playerset():    #setting up player class
	player = Player()
...
def dispinv():       #displaying inventory
    screen.addstr(15, 5, '+++++++++++++')
...
def disptop():       #diplaying top of home screen
	if floor == 'village':
...
def drawmap():           #draws the map using the function
	screen.addstr(6, 0, '\n'.join(''.join(str(cell) for cell in row) for row in currentmap))

def movedown():          #makes the floors move down and adds and deletes monsters to lvl
	global currentmap
	global floor
	global village
	global floor1
	...
	if floor == 'village':
		...
		floor = 'floor 2'
		currentmap = floor2
		Player.equipable['broadsword']['has'] = True
		currentmap[Player.playerY][Player.playerX] = '@'
		monster1 = Monster(False)
		...
		del monster2
		floor = 'floor 3'
		Player.equipable['shield']['has'] = True
		currentmap = floor3
		currentmap[Player.playerY][Player.playerX] = '@'
		monster1 = Monster(False)
		monster2 = Monster(False)
		...
		del monster3
		floor = 'floor 4'
		Player.equipable['big shield']['has'] = True
		currentmap = floor4
		currentmap[Player.playerY][Player.playerX] = '@'
		monster1 = Monster(False)
		monster2 = Monster(False)
...
def moveup():           #makes the floors move up and adds and deletes monsters to lvl
	global currentmap
	global floor
	global village
	global floor1
...
def elder():    #beggining person to talk for gives journal infromation
	global journal
...
def check(direction):  #checks the direction u mave and if there is something to interact with
    global currentmap
    global floor
    global village
    global floor1
...
def fightlist():  # monseters that are able to fight on each floor
	if floor == 'floor 1':
...
def Journal():     #puts joirnal on screen
	global journal
...
def inventory():     #displays inventory on game
	screen.clear()
	drawmap()
	disptop()
...
try:      #main function that runs everything
    while True:
        screen.clear()
        disptop()
        dispinv()
